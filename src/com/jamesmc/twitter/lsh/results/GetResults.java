...
public class GetResults implements Runnable {
	...
	public GetResults() {
		try {
			...
			this.connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/lshdb", "lsh", "lsh");
		...
		for (int i = 0; i < 4; i++) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						Class.forName("org.postgresql.Driver");
						connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/lshdb", "lsh", "lsh");
					...
					try {
						FileWriter fstream = new FileWriter("/home/jorge/lsh_output/lsh_events_final_4.output" + j);
